...
__global__ void setup_random_numbers(hiprandState *devStates,const unsigned long long seed,const int Np)
{
/************************************************************************
* Description:								*
*************************************************************************
...
//      hiprand_init(seed+id*rsrs, id, 0, &devStates[id]);
      hiprand_init(seed, id, 0, &devStates[id]);
      }
}
/*************************************************************************************************/


__global__ void config_init(hiprandState *devStates, dat2 *result, const int Np,const dat lx,const dat ly)
{
/************************************************************************
* Description:								*
*************************************************************************
...
************************************************************************/
  ...
  if(id<Np){
      ...
      hiprandState localState = devStates[id];
      /* Generate pseudo-random unsigned ints */

      /* free particles are distributed in the space above the substrate */
      if (id>=2*NAcut)
      {
	locpos.x = hiprand_uniform(&localState)*lx - 0.5*lx;
 	locpos.y = hiprand_uniform(&localState)*(ly-lyA)+lyA - 0.5*ly;
// 	locpos.y = hiprand_uniform(&localState)*(0.5*ly)+0.5*ly;
...
forces_calculation_tiles(hiprandState *devStates,dat2 *position, dat2 *dR, dat2 *devbin,const int Np,
			    ...
			    const dat delrxA, const dat delrxB)
{
  ...
  dat2 mydR=dR[ip];
  hiprandState localState = devStates[ip];
...
//
     ...
     dat2 rndnormal=hiprand_normal2(&localState);			/* get new random state */
     species = ip%2;
...
//     hipSetDevice(0);		/* 0 for Michelangelo */
...
//     fprintf(stderr,"rnd %d --> seed %u\n", rnd, seed[i]);
   ...
   ******************************************************/
//    /* For stream 1 */
   hiprandState *devStates[nstreams]; 	/* device copy of cuda (pseudo-)random states */
   ...
   ******************************************************/
   for (i=0; i<nstreams;i++) {
	CUDA_CALL(hipMalloc((void **)&devpos[i], csize)); 						/* allocate space for devpos on device (NP*sizeof(float2)) */
	CUDA_CALL(hipMalloc((void **)&devdR[i], csize));
	CUDA_CALL(hipMalloc((void **)&devbin[i], csize));
	CUDA_CALL(hipMalloc((void **)&devStates[i], (Np) * sizeof(hiprandState)));
   }
   /*****************************************************
   * Stream usage: Pinned memory required on host	*
   ******************************************************/
   for (i=0; i<nstreams;i++) {
	CUDA_CALL(hipHostMalloc((void **)&hostpos[i], csize));
	CUDA_CALL(hipHostMalloc((void **)&hostdR[i], csize));
   ...
   ******************************************************/
   for (i=0; i<nstreams;i++) {
      hipLaunchKernelGGL(setup_random_numbers, dim3(bpg), dim3(tpb), 0, stream[i], devStates[i],seed[i], (Np));
      hipLaunchKernelGGL(binary_init, dim3(bpg), dim3(tpb), 0, stream[i], devbin[i],Np);
   }
   /*****************************************************
   ...
   ******************************************************/
	...
	******************************************************/
	for (i=0; i<nstreams;i++) {
	    CUDA_CALL(hipMemsetAsync(devpos[i], 0.0, csize));
	    CUDA_CALL(hipMemsetAsync(devdR[i], 0.0, csize));
	    hipLaunchKernelGGL(config_init, dim3(bpg), dim3(tpb), 0, stream[i], devStates[i], devpos[i], Np,lx,ly); /* generate initial config on GPU */
	}
	/*****************************************************
	...
	******************************************************/
	for (i=0; i<nstreams;i++) {
	    ...
	    CUDA_CALL(hipMemcpyAsync(hostpos[i], devpos[i], csize, hipMemcpyDeviceToHost,stream[i]));
	    CUDA_CALL(hipMemcpyAsync(hostdR[i], devdR[i], csize, hipMemcpyDeviceToHost,stream[i]));
	}
	/*****************************************************
	* get timestep			 	 	     *
	******************************************************/
	...
	******************************************************/
	for (i=0; i<nstreams;i++){
	    ...
	    hipStreamSynchronize(stream[i]);	/* probably not necessary since read_config loop is only on CPU */
	    CUDA_CALL(hipMemcpyAsync(devpos[i], hostpos[i], csize, hipMemcpyHostToDevice, stream[i])); /* copy hostpos to device */
	    CUDA_CALL(hipMemcpyAsync(devdR[i], hostdR[i], csize, hipMemcpyHostToDevice, stream[i])); /* copy hostpos to device */
	    if (i==0) { printf(".. DONE\n"); }
	}
	timestep = hostpos[0][Np].x;
	printf("\n===> pickup time %lf\n\n",timestep);
   ...
   if (task == 3) {
	...
	for (i=0; i<nstreams;i++){
	    CUDA_CALL(hipMemsetAsync(devpos[i], 0, csize));					/* init devpos */

	    CUDA_CALL(hipMemcpyAsync(devpos[i], hostpos[i], csize, hipMemcpyHostToDevice,stream[i])); 		/* copy hostpos to device */

	    hipLaunchKernelGGL(config_cut, dim3(bpg), dim3(tpb), 0, stream[i], devpos[i], Np,lx,ly,invlx,invly); 	      		/* cut coordinates */

	    hipStreamSynchronize(stream[i]);
	    CUDA_CALL(hipMemcpyAsync(hostpos[i], devpos[i], csize, hipMemcpyDeviceToHost,stream[i])); 		/* copy devpos to host */

	    CUDA_CALL(hipMemsetAsync(devdR[i], 0, csize));						/* set dR to zero for a fresh start */

	    CUDA_CALL(hipMemcpyAsync(hostdR[i], devdR[i], csize, hipMemcpyDeviceToHost,stream[i])); 		/* copy devdR to host */
...
// 	    printf(".. DONE *stream %d* \n",i+1);
    ...
    */
    size_t free, total;
    printf("\n");
    hipMemGetInfo(&free,&total);	/* retrieve available memory information (bytes) */
    ...
    for(int iout=0;iout<nsteps;iout++)
    {
      ...
      for(int istep=0;istep<nout;istep++)
      {
	    ...
	    for (i=0; i<nstreams;i++){
		hipLaunchKernelGGL(forces_calculation_tiles, dim3(bpg), dim3(tpb), 0, stream[i], devStates[i],devpos[i],devdR[i],devbin[i],Np,lx,ly,vpcutbd,rcut,dt,Ggamma,sqtwodt,sqrtDB,devGO,invlx,invly,DB,shear_rate,delrxA,delrxB);
	    }
       ...
       for (i=0; i<nstreams;i++){
 	  hipLaunchKernelGGL(increment_time, dim3(bpg), dim3(tpb), 0, stream[i], devpos[i], Np,(dat)nout*dt); 	      		/* add time to coordinate pos[Np].x */
 	  hipLaunchKernelGGL(increment_time, dim3(bpg), dim3(tpb), 0, stream[i], devdR[i], Np,(dat)nout*dt);
       }

//        hipDeviceSynchronize();		/* without this barrier, results may be copied to host before the propagation step is complete */

      for (i=0; i<nstreams;i++){
 	  hipStreamSynchronize(stream[i]);	/* solves the problem of premature writing, too. */
	  CUDA_CALL(hipMemcpyAsync(hostpos[i], devpos[i], csize, hipMemcpyDeviceToHost,stream[i]));
	  CUDA_CALL(hipMemcpyAsync(hostdR[i], devdR[i], csize, hipMemcpyDeviceToHost,stream[i]));
	  idx[i] += 1;
// 	  dat2 temp;
...
/* Cleanup */
  for (i=0; i<nstreams;i++){
    CUDA_CALL(hipFree(devStates[i]));
    CUDA_CALL(hipFree(devpos[i]));
    CUDA_CALL(hipFree(devdR[i]));
